...
import { RowSelectionProvider } from "./row-selection/RowSelectionProvider";
import { SelectionBar } from "./row-selection/SelectionBar";
...
export function NotionDatabase({
  ...
  // Row ids in view order — feeds RowSelectionProvider (select-all + auto-deselect
  // of rows that leave the view via filter/delete).
  const rowOrder = useMemo(() => visibleRows.map((r) => r.id), [visibleRows]);
  ...
  return (
    <div className={cn("overflow-hidden rounded-lg border border-border bg-card", className)}>
      ...
      {/* Row multi-select: the provider enables the checkbox gutter + select-all
          in TableView (it reads useRowSelectionOptional); SelectionBar surfaces
          bulk actions once ≥1 row is picked. rowOrder auto-deselects filtered/
          deleted rows. */}
      <RowSelectionProvider rowOrder={rowOrder}>
        {!readOnly && onRowRemove && (
          <SelectionBar onDelete={(ids) => ids.forEach((id) => onRowRemove(id))} />
        )}
        <View
          db={db}
          view={activeView}
